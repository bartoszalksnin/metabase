...
(def ^:dynamic ^com.mongodb.DB *dynamodb-connection*
...
(defn- database->details
  ...
  (cond
    ...
    :else                         (throw (Exception. (str "with-dynamodb-connection failed: bad connection details:"
                                                          (:details database))))))

(defn -with-dynamodb-connection
    ...
    (ssh/with-ssh-tunnel [details-with-tunnel details]
      (let [{:keys [dbname host port user pass ssl authdb tunnel-host tunnel-user tunnel-pass additional-options]
            ...
            conn             (if credentials
                               (connect credentials)
                               (connect))
            dynamodb-connection (mg/get-db conn dbname)]
        (log/debug (u/format-color 'cyan "<< OPENED NEW MONGODB CONNECTION >>"))
        (try
          (binding [*dynamodb-connection* dynamodb-connection]
            (f *dynamodb-connection*))
          (finally        (mg/disconnect conn)
                          (log/debug (u/format-color 'cyan "<< CLOSED MONGODB CONNECTION >>"))))))))

(defmacro with-dynamodb-connection
  ...
  `(let [f# (fn [~binding]
     ...
     (if *dynamodb-connection*
       (f# *dynamodb-connection*)
       (-with-dynamodb-connection f# ~database))))
